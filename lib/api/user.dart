...
///  修改用户信息
Future<CommonResponse> editUserModelApi(UserModel user) async {
  String apiUrl = '${Api.AUTH_USER}/${user.id}';
  return await editData(apiUrl, user.toJson());
}

/// 保存用户信息
Future<CommonResponse> addUserModelApi(UserModel user) async {
  String apiUrl = Api.AUTH_USER;
  return await addData(apiUrl, user.toJson());
}

///  修改用户信息
Future<CommonResponse> removeUserModelApi(UserModel user) async {
  String apiUrl = '${Api.AUTH_USER}/${user.id}';
