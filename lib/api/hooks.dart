...
Future<CommonResponse<List<T>>> fetchDataList<T>(String apiEndpoint, T Function(Map<String, dynamic>) fromJson,
    {Map<String, dynamic>? queryParameters, CancelToken? cancelToken}) async {
  final response = await DioUtil().get(apiEndpoint, queryParameters: queryParameters, cancelToken: cancelToken);
  if (response.statusCode == 200) {
    // Logger.instance.d(response.data['data']);
    final dataList = ((response.data['data'] ?? []) as List).map<T>((item) => fromJson(item)).toList();
...
Future<CommonResponse> fetchBasicData<T>(String apiEndpoint,
  ...
  final response = await DioUtil().get(apiEndpoint, queryParameters: queryParameters, cancelToken: cancelToken);
  if (response.statusCode == 200) {
    return CommonResponse.fromJson(response.data, (p0) => p0 != null ? p0 as Map<String, dynamic> : p0);
  } else {
    String msg = '获取数据列表失败: ${response.statusCode}';
...
Future<CommonResponse> editData<T>(String apiUrl, Map<String, dynamic> formData,
  ...
  try {
    final response =
        await DioUtil().put(apiUrl, formData: formData, queryParameters: queryParameters, cancelToken: cancelToken);
    if (response.statusCode == 200) {
...
Future<CommonResponse> removeData(String apiUrl,
    {Map<String, dynamic>? queryParameters, CancelToken? cancelToken}) async {
  final response = await DioUtil().delete(apiUrl, queryParameters: queryParameters, cancelToken: cancelToken);
  if (response.statusCode == 200) {
    return CommonResponse.fromJson(response.data, (p0) => null);
  } else {
