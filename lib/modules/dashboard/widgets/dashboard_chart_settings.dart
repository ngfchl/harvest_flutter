...
class ChartSettingsDialog extends StatefulWidget {
  ...
  final void Function(List<String> order, Map<String, bool> visibility, double height, int treemapCount) onSaved;
...
class _ChartSettingsDialogState extends State<ChartSettingsDialog> {
  ...
  void initState() {
    ...
    _sliderController = FContinuousSliderController(selection: FSliderSelection(max: _toPercent(_chartHeight)));
  ...
  /// 实际高度 → 百分比 (0~1)
  double _toPercent(double height) => ((height - _min) / _range).clamp(0.0, 1.0);
  ...
  void _reset() {
    ...
    setState(() {
      ...
      _visibility = {for (final id in DashboardChartConfig.defaultOrder) id: true};
      _chartHeight = defaultHeight;
      _sliderController.selection = FSliderSelection(max: _toPercent(defaultHeight));
      _treemapSliderController.selection = FSliderSelection(max: _toTPercent(_treemapCount));
    });
  }
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text('仪表盘卡片设置', style: theme.typography.lg.copyWith(fontWeight: FontWeight.w700)),
                  ),
                  FButton.icon(
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Icon(FIcons.ruler, size: 14, color: theme.colors.mutedForeground),
                              const SizedBox(width: 6),
                              Text('图表高度', style: theme.typography.sm.copyWith(fontWeight: FontWeight.w600)),
                              const Spacer(),
                              Container(
                          ...
                          FSlider(
                            ...
                            onChange: (selection) {
                              setState(() => _chartHeight = _fromPercent(selection.offset.max));
                            },
                            tooltipBuilder: (ctrl, value) => Text('${_fromPercent(value).round()} px'),
                          ),
                          Row(
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Icon(FIcons.globe, size: 14, color: theme.colors.mutedForeground),
                              const SizedBox(width: 6),
                              Text('站点状态显示数量', style: theme.typography.sm.copyWith(fontWeight: FontWeight.w600)),
                              const Spacer(),
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                decoration: BoxDecoration(
                          ...
                          FSlider(
                            ...
                            onChange: (selection) {
                              setState(() => _treemapCount = _fromTPercent(selection.offset.max));
                            },
                            tooltipBuilder: (ctrl, value) => Text('${_fromTPercent(value)} 个'),
                          ),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Container(
                          ...
                          child: FTile(
                            ...
                            suffix: FSwitch(
                              value: visible,
                              onChange: (value) => setState(() => _visibility[id] = value),
                            ),
                            enabled: visible,
                          ),
                        );
