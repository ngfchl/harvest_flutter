...
class _SitePageState extends ConsumerState<SitePage> {
  ...
  Widget _buildToolbar(BuildContext context, int current, int total, bool hasFilters, bool mobile) {
    ...
    return AppSurfaceContainer(
      ...
      color: mobile ? appSurfaceColor(context, cs.background) : Colors.transparent,
      borderColor: mobile ? Colors.transparent : cs.border.withValues(alpha: 0.4),
      child: Row(
        spacing: mobile ? 6 : 8,
        children: [
        ...
        Row(
          children: [
            Container(
              width: 16,
              height: 16,
              decoration: BoxDecoration(color: siteSuccess(context, alpha: 0.18), shape: BoxShape.circle),
            ),
            const SizedBox(width: 5),
            Expanded(child: _previewLine(width: 28, color: cs.foreground, height: 6)),
            _previewLine(width: 18, color: siteSuccess(context, alpha: 0.18), height: 8),
          ],
        ),
        const SizedBox(height: 5),
        Row(
  ...
  Future<void> _showSiteTimeline(BuildContext context) async {
    final websites = ref.read(websiteListProvider).value ?? const <WebSite>[];
    final mySites = ref.read(siteInfoListProvider).value ?? const <SiteInfo>[];
    if (websites.isEmpty) {
    ...
    var ascending = true;
    final savedShowDurationOnTitle = HiveManager.get<bool>(StorageKeys.siteTimelineTitleShowDuration);
    ...
    await shadcn.showDialog<void>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setState) {
          ...
          final filteredEnabledOwned = enabledOwnedEntries.where(matches).toList()..sort(sortByTime);
          final filteredDisabledOwned = disabledOwnedEntries.where(matches).toList()..sort(sortByTime);
...
Widget _siteTimelineRow({
  ...
  return Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [BoxShadow(color: cs.foreground.withValues(alpha: 0.04), blurRadius: 12, offset: const Offset(0, 4))],
    ),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            Expanded(
              child: Text(
                entry.displayName,
                style: theme.typography.small.copyWith(fontWeight: FontWeight.w800, color: cs.foreground),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
            ),
        ...
        if (showStates) ...[
          ...
          Row(
            children: [
              if (entry.isDisabled) ...[_timelineStateTag(context, '已禁用'), const SizedBox(width: 6)],
              if (!entry.isOwned) ...[
...
Widget _timelineLinksIndicator(BuildContext context, List<String> urls) {
  ...
  final tooltip = availableUrls.isEmpty
      ? '暂无可访问链接'
      : ['可访问链接', for (final url in availableUrls) '${_timelineUrlHost(url)}\n$url'].join('\n\n');
  ...
  final child = shadcn.SecondaryBadge(
    child: Row(
      ...
      children: [
        ...
        Text(
          '$count',
          style: theme.typography.xSmall.copyWith(color: cs.foreground, fontWeight: FontWeight.w800, fontSize: 10.5),
        ),
      ],
    ),
  );
...
Widget _timelineTitleMeta(
  ...
  final child = Container(
    ...
    decoration: BoxDecoration(color: cs.muted.withValues(alpha: 0.24), borderRadius: BorderRadius.circular(999)),
    child: Row(
      mainAxisSize: MainAxisSize.min,
      children: [
...
class _TimelineMetric {
  ...
  const _TimelineMetric({required this.label, required this.value, required this.icon});
