            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  // ── 站点状态详情 ──
                  if (status != null) ...[const SizedBox(height: 12), _buildDetailStats(context, cs, status, spFull)],
                  const SizedBox(height: 12),
                  // ── 基本信息 ──
    ...
    return Column(
      ...
      children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(shadcn.LucideIcons.history, size: 12, color: cs.primary),
        ...
        Container(
          ...
          child: signed ? _buildSignedDetail(context, cs, displayText, todayTime) : _buildNotSigned(cs, context),
        ),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            if (time.isNotEmpty)
              Text(formatTime(time), style: TextStyle(fontSize: 10, color: cs.foreground.withValues(alpha: 0.35))),
            const Spacer(),
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(shadcn.LucideIcons.check, size: 11, color: siteSuccess(context)),
                  const SizedBox(width: 3),
                  Text(
                    '今日已签到',
                    style: TextStyle(fontSize: 11, color: siteSuccess(context), fontWeight: FontWeight.w600),
                  ),
                ],
              ),
            ),
        ...
        if (text.isNotEmpty) ...[
          const SizedBox(height: 8),
          Text(text, style: TextStyle(fontSize: 11, color: cs.foreground.withValues(alpha: 0.65), height: 1.5)),
        ],
      ],
    );
  }
      ...
      children: [
        Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(shadcn.LucideIcons.x, size: 11, color: siteWarning(context)),
              const SizedBox(width: 3),
              Text(
                '今日未签到',
                style: TextStyle(fontSize: 11, color: siteWarning(context), fontWeight: FontWeight.w600),
              ),
            ],
          ),
        ),
        ...
        shadcn.Button.outline(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [Icon(shadcn.LucideIcons.check, size: 14), const SizedBox(width: 4), const Text('签到')],
          ),
        ),
      ],
    );
            ...
            children: flags.map((f) {
              return SizedBox(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: f.on ? siteSuccess(context).withValues(alpha: 0.08) : cs.foreground.withValues(alpha: 0.03),
                    borderRadius: siteRadius(context, size: "md"),
                    border: Border.all(color: f.on ? siteSuccess(context).withValues(alpha: 0.2) : cs.border),
                  ),
                  child: Row(
                    ...
                    children: [
                      Icon(
                        f.icon,
                        size: 12,
                        color: f.on ? siteSuccess(context) : cs.foreground.withValues(alpha: 0.25),
                      ),
                      ...
                      Flexible(
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 11,
                            color: f.on ? siteSuccess(context) : cs.foreground.withValues(alpha: 0.4),
