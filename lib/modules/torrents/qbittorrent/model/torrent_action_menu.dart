...
typedef OnTorrentAction = Future<bool> Function(
...
class _MenuBody extends StatelessWidget {
    ...
    return [
      _section(context, [
        _tile(context,
          ...
          onTap: () => _exec(context,
              isPaused ? 'qb_resume' : 'qb_pause', {'hashes': [hash]}),
        ),
        _tile(context,
          ...
          onTap: () => _exec(context, 'qb_set_force_start',
              {'hashes': [hash], 'enable': !torrent.forceStart}),
        ),
      ]),
      _section(context, [
        _tile(context,
      ...
      _section(context, [
        _tile(context,
          ...
          onTap: () => _exec(context, 'qb_set_auto_management',
              {'hashes': [hash], 'enable': !torrent.autoTmm}),
        ),
        _tile(context,
        ...
        _tile(context,
          ...
          onTap: () => _exec(context, 'qb_set_super_seeding',
              {'hashes': [hash], 'enable': !torrent.superSeeding}),
        ),
      ]),
      _section(context, [
        _tile(context,
          ...
          onTap: () {
            ...
            _showConfirmDialog(context, '重新校验', '确定要重新校验种子吗？', () {
              _exec(context, 'qb_recheck', {'hashes': [hash]});
            });
          },
        ),
        _tile(context,
          ...
          label: '重新汇报',
          onTap: () => _exec(context, 'qb_reannounce', {'hashes': [hash]}),
        ),
        _tile(context,
        ...
        _tile(context,
          ...
          label: '导出 .torrent',
          onTap: () => _exec(
              context, 'qb_export', {'hashes': [hash], 'name': torrent.name}),
        ),
      ]),
    ];
  }
    ...
    return [
      _section(context, [
        _tile(context,
          icon: Icons.double_arrow_rounded,
          label: '强制开始',
          onTap: () => _exec(context, 'tr_start_now', {'ids': [hash]}),
        ),
        _tile(context,
          icon: Icons.play_arrow_rounded,
          label: '开始种子',
          onTap: () => _exec(context, 'tr_start', {'ids': [hash]}),
        ),
        _tile(context,
          icon: Icons.pause_rounded,
          label: '暂停种子',
          onTap: () => _exec(context, 'tr_stop', {'ids': [hash]}),
        ),
      ]),
      _section(context, [
        _tile(context,
      ...
      _section(context, [
        _tile(context,
          ...
          onTap: () {
            Navigator.pop(context);
            _showConfirmDialog(context, '重新校验', '确定要重新校验种子吗？', () {
              _exec(context, 'tr_verify', {'ids': [hash]});
            });
          },
        ),
        _tile(context,
          icon: Icons.campaign_outlined,
          label: '重新汇报',
          onTap: () => _exec(context, 'tr_reannounce', {'ids': [hash]}),
        ),
        _tile(context,
  ...
  Future<void> _exec(
      BuildContext context, String action, Map<String, dynamic> params) async {
    Navigator.pop(context);
    final success = await onAction(action, params);
    if (context.mounted) {
      _showToast(context, success ? '操作成功' : '操作失败',
          error: !success);
    }
  }
  ...
  Widget _chevron(FColors cs) => Icon(Icons.chevron_right_rounded,
      size: 16, color: cs.foreground.withOpacity(0.3));
      ...
      builder: (ctx) => _SubMenuBody(
        ...
        items: [
          ...
          _SubMenuItem(
              ...
              label: '哈希',
              onTap: () =>
                  _copy(ctx, torrent.hashString, '种子哈希')),
          _SubMenuItem(
              icon: FIcons.magnet,
              label: '磁力链接',
              onTap: () =>
                  _copy(ctx, torrent.magnetLink, '磁力链接')),
          _SubMenuItem(
              icon: Icons.link_rounded,
              label: 'Tracker 地址',
              onTap: () =>
                  _copy(ctx, torrent.trackerUrl, 'Tracker')),
          _SubMenuItem(
              icon: Icons.folder_outlined,
              label: '保存路径',
              onTap: () =>
                  _copy(ctx, torrent.downloadDir, '保存路径')),
        ],
      ),
    );
  }
      ...
      builder: (ctx) => _SubMenuBody(
        ...
        items: categories.map((cat) {
          ...
          return _SubMenuItem(
            ...
            iconColor:
            selected ? FTheme.of(ctx).colors.primary : null,
  ...
  void _showTagMenu(BuildContext context, String hash,
      {bool isTR = false}) {
    showFSheet(
      context: context,
      side: FLayout.btt,
      ...
      builder: (ctx) => _SubMenuBody(
        ...
        items: [
          _SubMenuItem(
              ...
              label: '队列顶部',
              onTap: () =>
                  _exec(ctx, 'tr_queue_top', {'ids': [hash]})),
          _SubMenuItem(
              icon: Icons.arrow_upward_rounded,
              label: '向上移动',
              onTap: () =>
                  _exec(ctx, 'tr_queue_up', {'ids': [hash]})),
          _SubMenuItem(
              icon: Icons.arrow_downward_rounded,
              label: '向下移动',
              onTap: () =>
                  _exec(ctx, 'tr_queue_down', {'ids': [hash]})),
          _SubMenuItem(
              icon: Icons.vertical_align_bottom_rounded,
              label: '队列底部',
              onTap: () =>
                  _exec(ctx, 'tr_queue_bottom', {'ids': [hash]})),
        ],
      ),
    );
  }
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text('同时删除文件',
                            style: TextStyle(
                                color: cs.foreground, fontSize: 13)),
                      ),
                      Switch(
                        value: deleteFiles,
                        onChanged: (v) =>
                            setDialogState(() => deleteFiles = v),
                      ),
                    ],
                  ),
  ...
  void _showLocationDialog(
      BuildContext context, String hash, String currentPath) {
  ...
  void _showUploadLimitDialog(
      BuildContext context, String hash, int currentLimit) {
    final ctrl = TextEditingController(
        text: currentLimit > 0
            ? (currentLimit / 1024).round().toString()
            : '');
                  ...
                  children: [
                    Text('限制分享率',
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.w600)),
                    const SizedBox(height: 16),
                    _radioOption(ctx, '使用全局分享率限制', -2.0,
                        ratioMode, (v) {
                          setDialogState(() => ratioMode = v);
                        }),
                    _radioOption(ctx, '无分享率限制', -1.0, ratioMode,
                            (v) {
                          setDialogState(() => ratioMode = v);
                        }),
                    _radioOption(ctx, '自定义分享率限制', 0.0, ratioMode,
                            (v) {
                          setDialogState(() => ratioMode = v);
                        }),
                    if (ratioMode == 0) ...[
  ...
  Widget _radioOption(BuildContext ctx, String label, double value,
      double groupValue, ValueChanged<double> onChanged) {
    final cs = FTheme.of(ctx).colors;
        ...
        child: Row(
          children: [
            Icon(
              ...
              color: selected
                  ? cs.primary
                  : cs.foreground.withOpacity(0.3),
            ),
            const SizedBox(width: 10),
            Text(label,
                style: TextStyle(
                    color: cs.foreground.withOpacity(0.8),
                    fontSize: 13)),
          ],
        ),
      ),
  ...
  void _showTrackerDialog(BuildContext context, String hash,
      {bool isTR = false}) {
    final ctrl = TextEditingController(
        text: torrent.trackerStats
            .map((t) => t.announce)
  ...
  void _showConfirmDialog(BuildContext context, String title,
  ...
  void _showToast(BuildContext context, String msg,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(msg,
            style: TextStyle(
                color: error ? Colors.white : cs.foreground,
                fontSize: 13)),
        backgroundColor:
        error ? const Color(0xFFEF4444) : cs.background,
