...
class QBittorrentController extends GetxController {
  ...
  List<Tracker> selectedTorrentTrackers = [];
  ...
  Future<void> initData() async {
    ...
    tags = await client.torrents.getTags();
  ...
  Future<void> controlTorrents({
    ...
    update();
    torrents = await client.torrents.getTorrentsList(options: const TorrentListOptions());
    update();
  }
  ...
  Future<CommonResponse> removeErrorTracker() async {
    try {
      ...
      showTorrents.removeWhere((element) => toRemoveTorrentList.contains(element.hash));
  ...
  Future<CommonResponse> replaceTrackers({required String site, required String newTracker}) async {
    ...
    try {
      for (String infoHash in hashes) {
        await replaceSingleTorrentTracker(infoHash: infoHash, newTracker: newTracker);
      }
    } catch (e, trace) {
      Logger.instance.e(e);
  ...
  Future<void> replaceSingleTorrentTracker({required String infoHash, required String newTracker}) async {
    List<Tracker> trackerList = await client.torrents.getTrackers(hash: infoHash);

    if (trackerList.isNotEmpty) {
      List<String> urls =
          trackerList.where((e) => e.url!.isNotEmpty && e.url!.startsWith('http')).map((e) => e.url!).toList();
      Logger.instance.d(urls);
      if (urls.isNotEmpty) {
        await client.torrents.editTracker(hash: infoHash, origUrl: urls[0], newUrl: newTracker);
      }
    }
  }
  ...
  Future<void> toggleSpeedLimit() async {
  ...
  TorrentSort? stringToTorrentSort(String? value) {
    ...
    return map[value] ?? TorrentSort.name; // 如果找不到对应的枚举值，则返回 TorrentSort.name
  }
}
